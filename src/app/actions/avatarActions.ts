...
import {
  PURCHASE_AN_AVATAR_ITEM,
  SET_FAVORITE,
} from 'api/mutations/collectibles';
...
export const doPurchaseAvatarItem = async (
  avatarId: number,
  studentId: number,
  token: string
) => {
...
export const doFetchFavoriteAvatars = async (
  studentId: number,
  token: string
) => {
  const res: any = await sendRawQuery(
...
export const doSetUserAvatar = async (
  ...
  try {
    ...
    return res.msg
      ...
      : {
          ...res.data.setCurrentFavoriteAvatarCollection
