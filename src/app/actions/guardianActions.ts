...
export const createGuardian = async(email: string, userName: string, password: string, dispatch: any) => {
...
export const createOrder = async(
    ...
    const res: any = await mutationFetch(
        CREATE_ORDER(discount, discountCode, guardianId, orderDetailInput, paymentMethod, returnUrl, subTotal, total),
        token
    ).catch(() => ({success: 'false'}));
    ...
    const { order, status, urlRedirect } = result.data.createOrder;
