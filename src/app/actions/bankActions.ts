...
export const withDraw = async (
  ...
  const res: any = await mutation(WITHDRAW(amount), token).catch(() => ({
    success: false,
  }));
...
export const deposit = async (amount: number, token: string, dispatch: any) => {
  const res: any = await mutation(DEPOSIT(amount), token).catch(() => ({
    success: false,
  }));
  if (res.success === false) {
    return {success: false, msg: 'Network Error'};
...
export const doFetchInterests = async () => {
  ...
  return res.data.interests ?? res.errors[0];
};
