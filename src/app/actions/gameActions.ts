...
export const getGameCategories = async (token: string) => {
  const res: any = await query('gamesCategory', GAMES_CATEGORY_QUERY, token).catch(() => ({ success: false }));
...
export const getGameByCategory = async (categoryName: string, token: string) => {
  const res: any = await query(`gamesByCategoryName(categoryName: "${categoryName}")`, GAMES_QUERY, token).catch(() => ({ success: false }));
...
export const playGame = async (gameId: number, token: string, dispatch: any) => {
  const res: any = await mutation(PLAY_GAME(gameId), token).catch(() => ({ success: false }));
  if (res.success === false) {
    return { success: false, msg: 'Network Error' };
  }
  ...
  const { student } = result.data.playGame
